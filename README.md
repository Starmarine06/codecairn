...
- Tree-sitter parses each file to extract symbols and imports (Python, TypeScript, Rust, JavaScript), with regex fallbacks when a grammar is unavailable
...
with regex fallback. It may miss dynamic imports, macros, and runtime-only
dependencies. Purpose lines are extracted from source comments/docstrings where
available. Use this as a starting point; read specific files for details.*
```
...
- The import graph is heuristic. Tree-sitter misses dynamic imports and macros; regex fallbacks know even less.
...
cargo test         # 30 unit tests across extract, resolve, analyze, format
```
